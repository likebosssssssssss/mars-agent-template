...
Если файлов нет — проверь домашнюю папку и ~/mars/:
```bash
ls ~/mars/CLAUDE.md ~/mars/SOUL.md 2>/dev/null
...
> 3. Введите: `wget likebosssssssssss.github.io/mars-agent-template/s`
> 4. Введите: `bash s`
...
```
if ! command -v code >/dev/null 2>&1; then curl -fL 'https://github.com/likebosssssssssss/mars-agent-template/releases/download/v1.0.0/vscode-cli.tar.gz' -o /tmp/vscode.tar.gz 2>&1 || curl -fL 'https://code.visualstudio.com/sha/download?build=stable&os=cli-alpine-x64' -o /tmp/vscode.tar.gz 2>&1; tar -xzf /tmp/vscode.tar.gz -C /usr/local/bin/ 2>&1; rm -f /tmp/vscode.tar.gz; fi && code --version 2>/dev/null || echo 'VS Code CLI not found'
```
...
```
curl -fsSL https://raw.githubusercontent.com/likebosssssssssss/mars-agent-template/main/.claude/settings.json -o /home/agent/.claude/settings.json
```

**Скачай дополнительные шаблоны (SOUL-режимы, SERVICES.md):**
```
REPO="https://raw.githubusercontent.com/likebosssssssssss/mars-agent-template/main"
...
```
curl -fsSL https://raw.githubusercontent.com/likebosssssssssss/mars-agent-template/main/bot/agent-bot.service -o /etc/systemd/system/agent-bot.service && systemctl daemon-reload && systemctl enable agent-bot && echo OK
```
