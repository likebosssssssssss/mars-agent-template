 ...
 * Part of mars-agent-template: personal AI agent architecture
 *
...
function buildSystemPrompt(isNewSession) {
  const parts = [];
  const today = new Date().toISOString().split("T")[0];

  // Полный пакет памяти (архитектура + DNA-файлы + дневники) нужен только
  // в момент СТАРТА новой сессии. При --resume Claude Code уже помнит его
  // из истории диалога — повторная отправка на каждое сообщение только
  // жгла токены без пользы.
  if (isNewSession) {
...
bot.command("update", async (ctx) => {
  ...
  try {
    ...
    const remoteVer = await new Promise((resolve, reject) => {
      https.get("https://raw.githubusercontent.com/likebosssssssssss/mars-agent-template/main/bot/VERSION", {
        timeout: 10000,
    ...
    if (!existsSync(updateScript)) {
      execSync(
        `curl -fsSL "https://raw.githubusercontent.com/likebosssssssssss/mars-agent-template/main/bot/update-bot.sh" -o "${updateScript}" && chmod +x "${updateScript}"`,
        { timeout: 15000 }
      );
    }
...
bot.command("connect", async (ctx) => {
  ...
  if (!existsSync(CODE_BIN)) {
    await ctx.reply(
      ...
      "<code>curl -sL https://raw.githubusercontent.com/likebosssssssssss/mars-agent-template/main/setup-server.sh | bash</code>",
      { parse_mode: "HTML" }
    );
    return;
  }
